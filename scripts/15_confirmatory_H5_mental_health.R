# ============================================================================
# Script 15: Confirmatory Test — H5: Sex-Differentiated Hidden Burden of
#             Mental and Behavioural Disorders
#
# PRE-REGISTERED ANALYSIS — follows OSF pre-registration.
#
# H5a (primary): Paired Wilcoxon signed-rank test comparing male vs female
#   MUR for mental/behavioural disorder sub-conditions (ICD-10 F00-F99).
#
# H5a (supplementary): Exact binomial sign test, permutation test (10,000),
#   bootstrap 95% CI (10,000 resamples).
#
# H5b (secondary): Repeat H5a separately for substance use disorders
...
#
# H5c (secondary): Temporal trend in sex gap using crude death rates
...
#   cause only, not MUR).
#
# INPUT: outputs/confirmatory/confirmatory_h5_data.csv   (cross-sectional MUR)
#        outputs/confirmatory/confirmatory_h5c_temporal.csv (crude rates 2014-2024)
#
# OUTPUT: outputs/confirmatory/h5_results.txt
#         outputs/confirmatory/h5_results.csv
#         outputs/figures/fig14_h5_mental_health_mur.png
#
...
cat("============================================================\n")
cat("Script 15: Confirmatory Test H5 -- Mental Health Hidden Burden\n")
cat("============================================================\n\n")
...
mur_file <- "outputs/confirmatory/confirmatory_h5_data.csv"
if (!file.exists(mur_file)) {
  stop("ERROR: ", mur_file, " not found. Run Scripts 12/12b first.")
}

h5_raw <- read_csv(mur_file, show_col_types = FALSE)
cat("H5 cross-sectional MUR data loaded:", nrow(h5_raw), "conditions\n")

# --- Temporal crude rate data (Cube 14, 2014-2024) ---
temporal_file <- "outputs/confirmatory/confirmatory_h5c_temporal.csv"
if (!file.exists(temporal_file)) {
  cat("WARNING: Temporal data not found. H5c trend analysis will be skipped.\n")
  h5c_temporal <- NULL
} else {
  h5c_temporal <- read_csv(temporal_file, show_col_types = FALSE)
  cat("Temporal data loaded:", nrow(h5c_temporal), "rows, years",
      min(h5c_temporal$year), "to", max(h5c_temporal$year), "\n")
}
...
cat("Conditions used in H5a paired test:\n")
...
if (nrow(h5_aggregate) > 0) {
  cat(sprintf("  Male MUR:    %.1f (underlying: %s, multiple: %s)\n",
              h5_aggregate$mur_male, format(h5_aggregate$underlying_male, big.mark = ","),
              format(h5_aggregate$multiple_male, big.mark = ",")))
  cat(sprintf("  Female MUR:  %.1f (underlying: %s, multiple: %s)\n",
              h5_aggregate$mur_female, format(h5_aggregate$underlying_female, big.mark = ","),
              format(h5_aggregate$multiple_female, big.mark = ",")))
  cat(sprintf("  Persons MUR: %.1f\n", h5_aggregate$mur_persons))
  cat(sprintf("  Male/Female ratio: %.2f\n",
              h5_aggregate$mur_male / h5_aggregate$mur_female))
  cat(sprintf("  Absolute difference: %.1f\n\n",
              h5_aggregate$mur_male - h5_aggregate$mur_female))
}

# ============================================================================
# H5a PRIMARY: Paired Wilcoxon Signed-Rank Test
# ============================================================================

cat("--- H5a: Paired Wilcoxon Signed-Rank Test ---\n\n")

diffs <- h5_blocks$mur_diff
...
wilcox_result <- wilcoxsign_test(
  h5_blocks$mur_male ~ h5_blocks$mur_female,
  distribution = "exact"
)

# Strip coin pvalue class immediately
h5a_statistic <- as.double(statistic(wilcox_result))
h5a_pvalue <- as.double(pvalue(wilcox_result))
h5a_z <- qnorm(h5a_pvalue / 2, lower.tail = FALSE)
h5a_effect_r <- h5a_z / sqrt(n_pairs)

cat("Wilcoxon signed-rank test (exact, two-tailed):\n")
cat("  Test statistic:", round(h5a_statistic, 3), "\n")
cat("  p-value:", format(h5a_pvalue, digits = 4, scientific = TRUE), "\n")
cat("  Effect size r:", round(h5a_effect_r, 3), "\n")
cat("  n pairs:", n_pairs, "\n\n")
...
# ---- H5a SUPPLEMENTARY: Exact Binomial Sign Test ----
cat("--- H5a Supplementary: Exact Binomial Sign Test ---\n\n")
...
# ---- H5a SUPPLEMENTARY: Permutation Test ----
cat("--- H5a Supplementary: Permutation Test ---\n\n")
...
# ---- H5a SUPPLEMENTARY: Bootstrap 95% CI ----
cat("--- H5a Supplementary: Bootstrap 95% CI ---\n\n")
...
# ============================================================================
# H5b SECONDARY: Substance Use vs Non-Substance
# ============================================================================

cat("=== H5b: Substance Use (F10-F19) vs Other Mental (F20-F99) ===\n\n")

# --- H5b Substance use: individual substance codes within F10-F19 ---
substance_codes <- h5_raw %>%
  filter(str_detect(icd_code, "^F1[0-9]$"),  # F10, F11, ..., F19
         !is.na(mur_male) & !is.na(mur_female)) %>%
  mutate(mur_diff = mur_male - mur_female)

cat("--- H5b: Substance Use Disorders (F10-F19) ---\n\n")
...
if (nrow(substance_codes) >= 3) {
  ...
  if (nrow(substance_codes) >= 4) {
    ...
    h5b_sub_stat <- as.double(statistic(sub_wilcox))
    h5b_sub_p <- as.double(pvalue(sub_wilcox))
    h5b_sub_r <- qnorm(h5b_sub_p / 2, lower.tail = FALSE) / sqrt(nrow(substance_codes))
  } else {
    # Too few for Wilcoxon, use sign test only
    h5b_sub_stat <- NA_real_
    sub_sign <- binom.test(n_sub_positive, nrow(substance_codes), p = 0.5)
    h5b_sub_p <- sub_sign$p.value
    h5b_sub_r <- NA_real_
  }

  cat(sprintf("  p-value: %s\n", format(h5b_sub_p, digits = 4)))
  if (!is.na(h5b_sub_r)) cat(sprintf("  Effect size r: %.3f\n", h5b_sub_r))
  cat(sprintf("  Mean diff: %.1f\n\n", mean(substance_codes$mur_diff)))

  # Note the aggregate direction
  sub_agg <- h5_raw %>% filter(icd_code == "F10-F19")
  if (nrow(sub_agg) > 0) {
...
# --- H5b Non-substance: block-level codes F20-F99 ---
non_sub_blocks <- h5_blocks %>%
  filter(!icd_code %in% c("F00-F09", "F10-F19"))  # F20+ blocks only

cat("--- H5b: Non-Substance Mental Disorders (F20-F99) ---\n\n")
...
if (nrow(non_sub_blocks) >= 3) {
  ...
  if (nrow(non_sub_blocks) >= 4) {
    ...
    h5b_nonsub_stat <- as.double(statistic(nonsub_wilcox))
    h5b_nonsub_p <- as.double(pvalue(nonsub_wilcox))
    h5b_nonsub_r <- qnorm(h5b_nonsub_p / 2, lower.tail = FALSE) /
      sqrt(nrow(non_sub_blocks))
  } else {
    h5b_nonsub_stat <- NA_real_
    nonsub_sign <- binom.test(n_nonsub_positive, nrow(non_sub_blocks), p = 0.5)
    h5b_nonsub_p <- nonsub_sign$p.value
    h5b_nonsub_r <- NA_real_
  }

  cat(sprintf("  p-value: %s\n", format(h5b_nonsub_p, digits = 4)))
  if (!is.na(h5b_nonsub_r)) cat(sprintf("  Effect size r: %.3f\n", h5b_nonsub_r))
  cat(sprintf("  Mean diff: %.1f\n\n", mean(non_sub_blocks$mur_diff)))
} else {
  cat("  Insufficient paired data for non-substance analysis.\n\n")
  h5b_nonsub_stat <- NA_real_
  h5b_nonsub_p <- NA_real_
  h5b_nonsub_r <- NA_real_
}

# H5b interpretation
cat("--- H5b Interpretation ---\n\n")
if (!is.na(h5b_sub_p) & !is.na(h5b_nonsub_p)) {
  if (h5b_sub_p < 0.05 & h5b_nonsub_p >= 0.05) {
    cat("  -> SUPPORTS H5b: Sex difference significant for substance use\n")
    cat("     but not for other mental disorders.\n\n")
  } else if (h5b_sub_p >= 0.05 & h5b_nonsub_p < 0.05) {
...
# ============================================================================
# H5c SECONDARY: Temporal Trend in Sex Gap (Crude Rates)
# ============================================================================

cat("--- H5c: Temporal Trend in Sex Gap (Crude Rates) ---\n\n")

h5c_cor <- NULL

if (!is.null(h5c_temporal)) {
  ...
  for (i in seq_len(nrow(h5c_overall))) {
    row <- h5c_overall[i, ]
    cat(sprintf("  %d    %8.2f      %8.2f        %.3f\n",
                row$year, row$male, row$female, row$mf_ratio))
  }

  h5c_cor <- cor.test(h5c_overall$year, h5c_overall$mf_ratio, method = "spearman",
                      exact = FALSE)
  ...
  direction <- ifelse(h5c_cor$estimate > 0, "NARROWING", "WIDENING")
  cat(sprintf("  Direction: Gap %s over time\n\n", direction))
} else {
  cat("  Temporal data not available. Skipping H5c.\n\n")
}
...
p14a <- ggplot(h5_plot_a) +
  ...
  coord_flip() +
  labs(
    title = "H5a: Mental Health MUR by Sex (2023)",
    subtitle = paste0("Wilcoxon p = ", format(h5a_pvalue, digits = 3),
...
if (!is.null(h5c_temporal)) {
  h5c_plot <- h5c_temporal %>%
    filter(icd_code == "F00-F99", sex != "persons")

  p14b <- ggplot(h5c_plot, aes(x = year, y = crude_rate_per_100k, colour = sex)) +
    ...
    labs(
      title = "H5c: Mental Health Underlying Death Rate by Sex (2014-2024)",
      subtitle = paste0("Spearman rho = ", round(h5c_cor$estimate, 3),
                        ", p = ", format(h5c_cor$p.value, digits = 4),
...
ggsave("outputs/figures/fig14_h5_mental_health_mur.png", p14_combined,
       width = 11, height = fig_height, dpi = 300, bg = "white")
cat("  -> Saved: outputs/figures/fig14_h5_mental_health_mur.png\n\n")
...
results_list <- list(
  tibble(
    ...
    test_type = "primary",
    statistic_name = "V",
    statistic_value = h5a_statistic,
    p_value_uncorrected = h5a_pvalue,
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    ci_method = NA_character_,
    effect_size_r = h5a_effect_r,
    n = n_pairs,
    significant_uncorrected = h5a_pvalue < 0.05,
  ...
  tibble(
    hypothesis = "H5b_substance",
    test = ifelse(is.na(h5b_sub_stat), "Sign test", "Wilcoxon signed-rank (exact)"),
    test_type = "secondary",
    statistic_name = ifelse(is.na(h5b_sub_stat), "n_positive", "V"),
    statistic_value = ifelse(is.na(h5b_sub_stat),
                              sum(substance_codes$mur_diff > 0), h5b_sub_stat),
    p_value_uncorrected = h5b_sub_p,
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    ci_method = NA_character_,
    effect_size_r = h5b_sub_r,
    n = nrow(substance_codes),
    significant_uncorrected = ifelse(is.na(h5b_sub_p), NA, h5b_sub_p < 0.05),
    deviation_note = "Substance use sub-conditions (F10-F19 individual codes)"
  ),
  tibble(
    hypothesis = "H5b_non_substance",
    test = ifelse(is.na(h5b_nonsub_stat), "Sign test", "Wilcoxon signed-rank (exact)"),
    test_type = "secondary",
    statistic_name = ifelse(is.na(h5b_nonsub_stat), "n_positive", "V"),
    statistic_value = ifelse(is.na(h5b_nonsub_stat),
                              sum(non_sub_blocks$mur_diff > 0), h5b_nonsub_stat),
    p_value_uncorrected = h5b_nonsub_p,
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    ci_method = NA_character_,
    effect_size_r = h5b_nonsub_r,
    n = nrow(non_sub_blocks),
    significant_uncorrected = ifelse(is.na(h5b_nonsub_p), NA, h5b_nonsub_p < 0.05),
    deviation_note = "Non-substance block-level conditions (F20-F99)"
  )
)

# Add H5c if available
if (!is.null(h5c_cor)) {
  results_list[[length(results_list) + 1]] <- tibble(
    ...
    test_type = "secondary",
    statistic_name = "rho",
    statistic_value = as.numeric(h5c_cor$estimate),
    p_value_uncorrected = h5c_cor$p.value,
    ci_lower = NA_real_,
    ci_upper = NA_real_,
    ci_method = NA_character_,
    effect_size_r = as.numeric(h5c_cor$estimate),
    n = nrow(h5c_overall),
    significant_uncorrected = h5c_cor$p.value < 0.05,
    deviation_note = "Uses crude death rates (not MUR) due to data availability"
  )
}

h5_results <- bind_rows(results_list)
write_csv(h5_results, "outputs/confirmatory/h5_results.csv")

# Human-readable results
sink("outputs/confirmatory/h5_results.txt")
cat("============================================================\n")
...
if (nrow(h5_aggregate) > 0) {
  ...
  cat(sprintf("    Male:    %.1f\n", h5_aggregate$mur_male))
  cat(sprintf("    Female:  %.1f\n", h5_aggregate$mur_female))
  cat(sprintf("    Ratio:   %.2f (male MUR is %.0f%% higher)\n",
              h5_aggregate$mur_male / h5_aggregate$mur_female,
              (h5_aggregate$mur_male / h5_aggregate$mur_female - 1) * 100))
}
cat("\n")

cat("H5a PRIMARY: Paired Wilcoxon Signed-Rank\n")
cat("  Null: No difference in male vs female MUR for mental disorders\n")
cat("  Test statistic V:", round(h5a_statistic, 3), "\n")
cat("  p-value (exact, two-tailed):", format(h5a_pvalue, digits = 6), "\n")
cat("  Effect size r:", round(h5a_effect_r, 3), "\n")
...
cat("H5b SECONDARY: Substance Use (F10-F19)\n")
cat("  n conditions:", nrow(substance_codes), "\n")
cat("  p-value:", format(h5b_sub_p, digits = 6), "\n")
if (!is.na(h5b_sub_r)) cat("  Effect size r:", round(h5b_sub_r, 3), "\n")
cat("  Mean diff:", round(mean(substance_codes$mur_diff), 1), "\n\n")

cat("H5b SECONDARY: Non-Substance (F20-F99)\n")
cat("  n conditions:", nrow(non_sub_blocks), "\n")
cat("  p-value:", format(h5b_nonsub_p, digits = 6), "\n")
if (!is.na(h5b_nonsub_r)) cat("  Effect size r:", round(h5b_nonsub_r, 3), "\n")
cat("  Mean diff:", round(mean(non_sub_blocks$mur_diff), 1), "\n\n")

if (!is.null(h5c_cor)) {
  cat("H5c SECONDARY: Temporal Trend (crude death rates, not MUR)\n")
  cat("  Spearman rho:", round(h5c_cor$estimate, 3), "\n")
  cat("  p-value:", format(h5c_cor$p.value, digits = 6), "\n")
  cat("  n years:", nrow(h5c_overall), "\n")
  cat("  Direction:", ifelse(h5c_cor$estimate > 0, "Gap narrowing", "Gap widening"), "\n\n")
}
...
cat("  -> Saved: outputs/confirmatory/h5_results.csv\n")
cat("  -> Saved: outputs/confirmatory/h5_results.txt\n\n")
