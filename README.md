...
|----|-----------|-------------|--------|
| H1 | Hypertension ratio differs by sex | Wilcoxon signed-rank | Not supported (p_holm = 0.25) |
| H2 | Avoidable conditions have higher geographic CV | Mann-Whitney U | Not supported (p_holm = 0.87) |
| H3 | Mental health ratio differs by sex, driven by substance use | Wilcoxon signed-rank | Not supported (p_holm = 0.07) |
...
| `12c_temporal_pbs_ed_cv.R` | Cube 14 temporal data, PBS fix, ED data, full ratio table |
| `12d_fix_issues.R` | Fix H2 rate conversion, population adjustment, avoidability merge |

### Phase 5: Confirmatory Tests
| Script | Description |
|--------|------------|
| `13_confirmatory_H1_hypertension.R` | H1: Hypertension ratio sex difference |
| `14_confirmatory_H2_geographic.R` | H2: Geographic variation by avoidability |
| `15_confirmatory_H3_mental_health.R` | H3: Mental health ratio sex difference |
| `16_holm_bonferroni_summary.R` | Holm-Bonferroni correction across all tests |

### Phase 6: Exploratory Extensions
| Script | Description |
|--------|------------|
| `17_mur_ranking_table.R` | Full ratio ranking for 663 conditions |
| `18_hospital_mortality_linkage.R` | Hospital procedure volume vs ratio linkage |
| `19_pbs_vs_mortality.R` | PBS prescribing vs hypertension mortality trends |
| `20_temporal_high_mur.R` | Temporal trends for high-ratio conditions |
| `21_fig1_mur_concept.R` | Figure 1: Conceptual ratio illustration |
...
Complete ratio rankings, temporal trends, PBS prescribing linkage, and hospital-mortality concordance.
...
- **Hypertension ratio = 27.7** (41.8 males, 21.2 females) — the largest hidden burden among common conditions
- **663 conditions ranked by ratio** from 1.0 (external causes like self-harm) to 281.1 (surgical complications)
- **71 conditions with ratio > 10** reveal conditions where standard mortality statistics capture < 10% of the true burden
